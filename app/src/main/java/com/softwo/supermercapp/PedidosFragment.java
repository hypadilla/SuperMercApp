...
public class PedidosFragment extends Fragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        mRecyclerViewPedidos.setLayoutManager( mLayoutManagerPedidos );

        mAdapterPedidos = new AdaptadorPedidos();

        mRecyclerViewPedidos.setAdapter( mAdapterPedidos );
    ...
    private void getPedidos() {
        ...
        Cursor cursor = databaseHelper.ConsultarPedidos();

        if (cursor.moveToFirst()) {
            do {
                Query query = FirebaseDatabase.getInstance().getReference( FireBase.BASEDATOS )
                        .child( FireBase.TABLAPEDIDO )
                        .orderByKey()
                        .equalTo(cursor.getString( cursor.getColumnIndex( PedidosContract.PedidoEntry.ID ) ));

                query.addValueEventListener( new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        List<Pedidos> productos = new ArrayList<>();
                        for (DataSnapshot userSnapshot : dataSnapshot.getChildren()) {
                            Pedidos producto = userSnapshot.getValue( Pedidos.class );
                            productos.add( producto );
                        }
                        ((AdaptadorPedidos) mAdapterPedidos).addAll( productos );
                    }
        ...
        //((AdaptadorPedidos) mAdapterPedidos).addAll( pedidos );
